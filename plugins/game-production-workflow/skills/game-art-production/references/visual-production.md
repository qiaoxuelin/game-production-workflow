...
Use for character, environment, 3D, animation, VFX, technical art, and broad
visual work inside the core-owned task. Select only the supplied operation:
`Design` proposes the brief, craft rules, inventory, and proof criteria;
`Produce` consumes the core-frozen versions and implements them;
`Review` inspects conformance read-only. Instructions to freeze, record packages,
or unlock work are recommendations to core, never independent art authority.
Do not repeat a satisfied design step in Produce or edit assets in Review.
One accountable owner may cover adjacent disciplines; do not create extra
agents, documents, or approvals merely to name every concern.
...
- for player-operated surfaces, player action, input, source/target, pre/post
  state, visible feedback, and failure/recovery; noninteractive composition
  needs only its render/spatial contract, not an invented interaction matrix;
...
For `Produce`, integrate the representative slice before polish. If findings
remain, select one dominant root cause and make one bounded repair batch total
for the operation, then run the affected regression and return the existing
transient handoff to core. Preserve the same-root history through
[returned-candidate convergence](../../game-production-system/references/execution.md#returned-candidate-convergence).
Only an actual integrated result supports `Implemented`; missing observation
cannot support runtime or quality claims. Keep bulk locked without representative
assembly and runtime proof. Design and Review return candidates or findings
without implementing or granting acceptance.
